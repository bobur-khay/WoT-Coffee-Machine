...
class HttpServer {
    ...
    addEndpoint(thing, tdTemplate, base) {
        for (const type of core_1.ContentSerdes.get().getOfferedMediaTypes()) {
            ...
            for (const [propertyName, property] of Object.entries(thing.properties)) {
                ...
                const form = new core_1.Form(href, type);
                core_1.ProtocolHelpers.updatePropertyFormWithTemplate(form, (tdTemplate.properties?.[propertyName] ?? {}));
                const readOnly = property.readOnly ?? false;
                const writeOnly = property.writeOnly ?? false;
                if (readOnly) {
            ...
            for (const [actionName, action] of Object.entries(thing.actions)) {
                ...
                const form = new core_1.Form(href, type);
                core_1.ProtocolHelpers.updateActionFormWithTemplate(form, (tdTemplate.actions?.[actionName] ?? {}));
            ...
            for (const [eventName, event] of Object.entries(thing.events)) {
                ...
                const form = new core_1.Form(href, type);
                core_1.ProtocolHelpers.updateEventFormWithTemplate(form, (tdTemplate.events?.[eventName] ?? {}));
                form.subprotocol = "longpoll";
    ...
    async checkCredentials(thing, req) {
